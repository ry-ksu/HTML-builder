  ...
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          fsPromises.writeFile(path.resolve(__dirname, 'project-dist', 'index.html'), dataTemplate);
        } catch (err) {
          throw err;
        }
      } catch (err) {
    ...
    for (let file of filesStyle) {
      try {
        if (path.extname(file.name) === '.css' && file.isFile() === true) {
          try {
            let fileStyle = await fsPromises.readFile(path.resolve(__dirname, 'styles', file.name), 'utf-8');
